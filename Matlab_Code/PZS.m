function [data_2D,output]=PZS(N,freq_r,norder,nExpDec,datatype)
...
% Select the specific folder where 2DIR matlab figures are saved.
...
%% Analysis of single component system (non-overlapping spectral bands)       
    case 'single'
        ...
        if nargin < 3 || isempty(freq_r)
            ...
            [~,max_loc]=max(S0_diag);
            peak_ratio=0.1;
            [r_max]=find(S0_diag>(min(S0_diag)+peak_ratio*range_calc(S0_diag)));
       ...
       %    [tinv,~,~]=ExpFunFit([Tw,ffcf(ji,:)'],nExpDec,min(ffcf(ji,:)));
           tau(ji,1)= TimeDecay(ffcf(ji,:)',Tw,nExpDec,1);
            Pv{ji,1}=sm;
...
%% Create figures for frequency window optimization and FFCF results.
        ...
        box on;
        axis tight;
        set(gca,'Ylim',[min(output.analysis.tau_in_ps)-0.1*range_calc(output.analysis.tau_in_ps) max(output.analysis.tau_in_ps)+0.1*range_calc(output.analysis.tau_in_ps)]);
        ...
        box on;
        axis tight;
        set(gca,'Ylim',[min(ffcf0)-0.1*range_calc(ffcf0) max(ffcf0)+0.1*range_calc(ffcf0)]);
...
for k=mp-ur1:mp+ur2
    ...
    n=n+1;
            ...
            for k=1:p
                cp=similarity(pz(:,:,k)');
                ffcf(k,:)=cp;
                tau(k,1)=TimeDecay(cp,Tw,nExpDec,1);
            end
            output.CL_PZS=ffcf;
        ...
        box on;
        axis tight;
        [fn,S]=polyfit(CLS_m,ffcf0,1);
        yfit=polyval(fn,CLS_m);
        hold on, plot(CLS_m,yfit,'r-','LineWidth',2);hold off
        r2=1-(S.normr/norm(ffcf0 - mean(ffcf0)))^2;
        r2=sprintf('%.4f',r2);
        text(min(CLS_m),max(ffcf0),['R^2 = ',r2],'Fontsize',14,'HorizontalAlignment','left','VerticalAlignment',"top");
        legend off;
        set(gca,'Ylim',[min(ffcf0)-0.1*range_calc(ffcf0) max(ffcf0)+0.1*range_calc(ffcf0)]);
        set(gca,'Xlim',[min(CLS_m)-0.1*range_calc(CLS_m) max(CLS_m)+0.1*range_calc(CLS_m)]);
        set(gcf, 'position',[figV figH figL figW]);
    ...
    case 'overlapping' 
        ...
        if nargin < 3 || isempty(freq_r)
            ...
            S_p=Smax(nrow_c,ncol_c,:); 
                    ...
                    if nargin > 5 && strcmp(datatype,"psc")==1
                            p_shift=nan(n_Tw,2);
         ...
         wtau_c=wtau(nrow_c);
            ...
            if length(freq_r)==2
                nrow_c=find(wtau>freq_r(1) & wtau<freq_r(2));
                ncol_c=find(wt>freq_r(1) & wt<freq_r(2));
            elseif length(freq_r)==4
                nrow_c=find(wtau>freq_r(3) & wtau<freq_r(4));
                ncol_c=find(wt>freq_r(1) & wt<freq_r(2));
            end
                    ...
                    if nargin > 5 && strcmp(datatype,"psc")==1
                        ...
                        for j=1:n_Tw
                          %  [nr,nc]=find(Smax(:,:,j)==1);
                            ...
                            p_shift(j,:)=[wtau(nr),wt(nc)];
                            p_diff=round([wtau(nr),wt(nc)]-Pk_freq);
                            if length(freq_r)==2
                                nrow_c=find(wtau>(freq_r(1)+p_diff(1)) & wtau<(freq_r(2)++p_diff(2)));
                                ncol_c=find(wt>(freq_r(1)+p_diff(2)) & wt<(freq_r(2)+p_diff(2)));
                            elseif length(freq_r)==4
                                nrow_c=find(wtau>(freq_r(3)+p_diff(1)) & wtau<(freq_r(4)+p_diff(1)));
                                ncol_c=find(wt>(freq_r(1)+p_diff(2)) & wt<(freq_r(2)+p_diff(2)));
                            end
                           S_p(:,:,j)=Smax(nrow_c,ncol_c,j);
                        end
...
%% Create figures for frequency resolved analysis and FFCF results.
        ...
        ylabel('fitted \tau_c (ps)');
        xlabel('Center frequency (cm^-^1)');
        title ('FR-PZS (Large Area)');
        set(gca,'FontSize',14);
        box on;
        axis tight;
        set(gca,'Ylim',[min(tau0)-0.05*range_calc(tau0) max(tau0)+0.05*range_calc(tau0)]);
        ...
        set(gca,'YColor','b');
        set(gca,'FontSize',14);
        set(gca,'Ylim',[min(ffcf1)-0.05*range_calc(ffcf1) max(ffcf1)+0.05*range_calc(ffcf1)]);
        set(gca,'Ylim',[min(ffcf1)-0.05*range_calc(ffcf1) max(ffcf1)+0.05*range_calc(ffcf1)]);
        if length(id)>1
        ...
        xlabel('waiting time (ps)');
        title 'FR-PZS : FFCF (Large Area)';
        set(gca,'FontSize',14);
        box on;
        axis tight;
        legend('Low Frequency','High Frequency');
        yyaxis left, set(gca,'Ylim',[min(ffcf1)-0.05*range_calc(ffcf1) max(ffcf1)+0.05*range_calc(ffcf1)]);
        yyaxis right, set(gca,'Ylim',[min(ffcf2)-0.05*range_calc(ffcf2) max(ffcf2)+0.05*range_calc(ffcf2)]);
...
if tl>1
    ...
    if npf == 1 && diff_percent > 0.2
        ...
        dr=size(Sp,4);
            ...
            if rem(mp,2)==0
                pc_hf=PZP_calc(rl_hf+1,norder);
                Sp_hf=Sp(mp-floor(mp/2)+pd/2:mp+ceil(mp/2)-pd/2,mp+pd:end,:,ind);
            else
            ...
            tau_hf=nan(p,1);
                ...
                ffcf_hf(k,:)=cp;
                tau_hf(k,1)=TimeDecay(cp,Tw,nExpDec,1);
                end
    ...
    %% Low Frequency Analysis
           ...
           if length(taus)>1
            ...
            if rem(mp,2)==0
                pc_lf=PZP_calc(rl_lf+1,norder);
                Sp_lf=Sp(mp-floor(mp/2)+pd/2:mp+ceil(mp/2)-pd/2,1:mp+1-pd,:,ind);
                wt_lf=wt_c(1:mp+1-pd);
            else
            ...
            tau_lf=nan(p,1);
                ...
                ffcf_lf(k,:)=cp;
                tau_lf(k,1)=TimeDecay(cp,Tw,nExpDec,1);
                end
    ...
    %% Create figures for in-depth frequency resolved analysis and FFCF results.
            ...
            ylabel('fitted \tau_c (ps)');
            xlabel('Center frequency (cm^-^1)');
            title ('FR-PZS (Small Area)');
            set(gca,'FontSize',14);
            box on;
            axis tight;
            set(gca,'Ylim',[min([tau_sh;tau_sl])-0.05*range_calc([tau_sh;tau_sl]) max([tau_sh;tau_sl])+0.05*range_calc([tau_sh;tau_sl])]);
...
function [tau_c]=TimeDecay(SM,t,n,intcpt)
if nargin<2, intcpt = 0; end
x  = t/1000;
nx = size(x, 1);
y  = SM;
% Calculate integrals
...
A = pinv(Y)*y;
lambdas = eig([A(1:n)'; eye(n-1), zeros(n-1,1)]);
tau_c=-1./lambdas;
end


function [out]=range_calc(input)
out=max(input)-min(input);
end
